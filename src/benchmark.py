...
class ModelParams():
    ...
    def create_param_dict(self) -> Dict[str, Any]:
        return {
            ...
            "robot_initial_placements": self.robot_initial_placements if isinstance(self.robot_initial_placements, list) else [self.robot_initial_placements],
            "allowed_to_switch": self.allowed_to_switch,
            "gamma": self.gamma,
            "k": self.k,
            "m": self.m,
            "switching_cost": self.switching_cost,
            "delay_random_range": self.delay_random_range if isinstance(self.delay_random_range, list) else [self.delay_random_range],
...
def main():
    params = ModelParams()
    params.n_tasks = 1
    params.task_dist_calc = list(range(10, 80, 10))
    # params.m = list(range(2,20,1))
    # params.k = list(range(1,15,1))
    # params.gamma = list(range(0.01, 0.3, 0.01))
    
    run_and_save(params, "results.tsv", 8, itterations_per_combo=1)
