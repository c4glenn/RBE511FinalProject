...
def main():
    params = ModelParams()
    params.n_tasks = [4]
    params.task_distribution = [np.array([30,25,20,15,10]),
                                np.array([25,20,10,20,25]),
                                np.array([10,20,40,20,10]),
                                np.array([10,15,20,25,30])]
    
    run_and_save(params, "results_dist.tsv", 5) #itterations_per_combo=1)
